...
[![Build Status](https://secure.travis-ci.org/adelevie/av_resource.png)](http://travis-ci.org/adelevie/av_resource) [![Code Climate](https://codeclimate.com/github/adelevie/av_resource.png)](https://codeclimate.com/github/adelevie/av_resource)


AVResource makes it easy to interact with cn.avoscloud.com's REST API. It adheres to the ActiveRecord pattern. AVResource is fully ActiveModel compliant, meaning you can use validations and Rails forms.
...
Use cases
-------------
*   Build a custom admin dashboard for your cn.avoscloud.com data
...
gem install av_resource
```

Create an account at cn.avoscloud.com. Then create an application and copy the `app_id` and `master_key` into a file called `av_resource.yml`. If you're using a Rails app, place this file in the `config` folder.
...
If you keep `avos_resource.yml` in `.gitignore`, AVResource will alternatively look for the api keys in environment variables. If using Heroku you can easily set your api keys in the Heroku environment using:
...
You can create separate AV databases if you want. If not, include the same info for each environment.

In a non-Rails app, include this somewhere (preferable in an initializer):


```ruby
AVResource::Base.load!("your_app_id", "your_master_key")
```
...
If you are using version `1.5.11` or earlier, subclass to just `AVResource`--or just update to the most recent version.
...
p.title = "Introducing AVResource" #=> "Introducing AVResource" 
p.valid? #=> true 
...
# checking the id generated by AV's servers
p.id #=> "QARfXUILgY" 
p.updated_at #=> nil 
p.created_at #=> "2011-09-19T01:32:04.973Z" # does anybody want this to be a DateTime object? Let me know.

# updating
p.title = "[Update] Introducing AVResource"
p.save #=> true
p.updated_at #=> "2011-09-19T01:32:37.930Z" # more magic from AV's servers
...
# nothing gets sent to the AV server until you run #all, #count, or any Array method on the query 
...
# destroy all objects, updated to use AV batch destroy
...
Note: Because users are special in the AV API, you must name your class User if you want to subclass AVUser.
...
class User < AVUser
  ...
  # but note that email is a special field in the AV API.
  fields :email
end
...
user.save #=> true
# after saving, the password is automatically hashed by AV's server
...
If you want to use av_resource to back a simple authentication system for a Rails app, follow this [tutorial](http://asciicasts.com/episodes/250-authentication-from-scratch), and make some simple modifications.

Installations

Note: Because [Installations](https://parse.com/docs/rest#installations), are special in the AV API, you must name your class Installation if you want to manipulate installation objects.

```ruby
class Installation < AVResource::Base
...
```ruby
class Place < AVResource::Base
  fields :location
end

place = Place.new
place.location = AVGeoPoint.new :latitude => 34.09300844216167, :longitude => -118.3780094460731
place.save
place.location.inspect #=> #<AVGeoPoint:0x007fb4f39c7de0 @latitude=34.09300844216167, @longitude=-118.3780094460731>


place = Place.new
place.location = AVGeoPoint.new
...
server_place.location.inspect #=> #<AVGeoPoint:0x007fb4f39c7de0 @latitude=34.09300844216167, @longitude=-118.3780094460731>
...
Documentation
-------------
[Here](http://rubydoc.info/gems/av_resource/)
...
User authentication is my top priority feature. Several people have specifically requested it, and AV just began exposing [User objects in the REST API](https://www.parse.com/docs/rest#users).

Let me know of any other features you want.


Contributing to AVResource
-----------------------------
...
*   Create `av_resource.yml` in the root of the gem folder. Using the same format as `av_resource.yml` in the instructions (except only creating a `test` environment, add your own API keys.
