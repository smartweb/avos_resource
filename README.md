...
AVOSResource makes it easy to interact with cn.avoscloud.com's REST API. It adheres to the ActiveRecord pattern. AVOSResource is fully ActiveModel compliant, meaning you can use validations and Rails forms.
...
Use cases
-------------
*   Build a custom admin dashboard for your AVOS.com data
...
Create an account at AVOS.com. Then create an application and copy the `app_id` and `master_key` into a file called `parse_resource.yml`. If you're using a Rails app, place this file in the `config` folder.
...
If you keep `avos_resource.yml` in `.gitignore`, AVOSResource will alternatively look for the api keys in environment variables. If using Heroku you can easily set your api keys in the Heroku environment using:
...
You can create separate AVOS databases if you want. If not, include the same info for each environment.

In a non-Rails app, include this somewhere (preferable in an initializer):


```ruby
AVOSResource::Base.load!("your_app_id", "your_master_key")
```
...
If you are using version `1.5.11` or earlier, subclass to just `AVOSResource`--or just update to the most recent version.
...
p.title = "Introducing AVOSResource" #=> "Introducing AVOSResource" 
p.valid? #=> true 
...
# checking the id generated by AVOS's servers
p.id #=> "QARfXUILgY" 
p.updated_at #=> nil 
p.created_at #=> "2011-09-19T01:32:04.973Z" # does anybody want this to be a DateTime object? Let me know.

# updating
p.title = "[Update] Introducing AVOSResource"
p.save #=> true
p.updated_at #=> "2011-09-19T01:32:37.930Z" # more magic from AVOS's servers
...
# nothing gets sent to the AVOS server until you run #all, #count, or any Array method on the query 
...
# destroy all objects, updated to use AVOS batch destroy
...
Note: Because users are special in the AVOS API, you must name your class User if you want to subclass AVOSUser.
...
class User < AVOSUser
  ...
  # but note that email is a special field in the AVOS API.
  fields :email
end
...
user.save #=> true
# after saving, the password is automatically hashed by AVOS's server
...
Note: Because [Installations](https://parse.com/docs/rest#installations), are special in the AVOS API, you must name your class Installation if you want to manipulate installation objects.

```ruby
class Installation < AVOSResource::Base
...
```ruby
class Place < AVOSResource::Base
  fields :location
end

place = Place.new
place.location = AVOSGeoPoint.new :latitude => 34.09300844216167, :longitude => -118.3780094460731
place.save
place.location.inspect #=> #<AVOSGeoPoint:0x007fb4f39c7de0 @latitude=34.09300844216167, @longitude=-118.3780094460731>


place = Place.new
place.location = AVOSGeoPoint.new
...
server_place.location.inspect #=> #<AVOSGeoPoint:0x007fb4f39c7de0 @latitude=34.09300844216167, @longitude=-118.3780094460731>
...
User authentication is my top priority feature. Several people have specifically requested it, and AVOS just began exposing [User objects in the REST API](https://www.parse.com/docs/rest#users).

Let me know of any other features you want.


Contributing to AVOSResource
-----------------------------
